...
static safety_config tesla_init(uint16_t param) {
  ...
  // moonpilot seam, see AGENTS.md: the lateral-only permission. Value 4 (bit 2); bit 0 is
  // TESLA_FLAG_LONGITUDINAL_CONTROL and bit 1 is the deprecated DAS_STEERING_3_BIT flag. Host-armed,
  // not switch-armed: the mode decodes no cruise main switch, so openpilot's own engaged heartbeat
  // is the arm (see moonpilot/lateral_engage.h). Tesla's rx hook is also the fork's only source of
