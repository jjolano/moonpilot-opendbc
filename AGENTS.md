...
- **The rule's steering-override term is inert on the fork's only supported brand.** `lateral_engage_update` clears the permission on the rising edge of upstream's `steering_disengage`, and only `modes/tesla.h` ever sets that signal, so no Toyota reaches the branch. The fork's safety test injects the flag to hold the rule itself; do not read it as a disarm that happens on the road, and do not add a Toyota equivalent without deciding that driver torque should stop steering — upstream's own answer there is to blend it.
...
- **The mixin's steering-disengage case is injected, not observed.** `test_lateral_engage_disarms_on_steering_disengage` drives `steering_disengage` through the harness because no Toyota sets it (previous bullet); it holds the rule, not a road behavior. Keep its docstring saying so.
